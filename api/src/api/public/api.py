...
from api.shared.api.dependencies import get_ro_session, get_rw_session
from api.public.email_sender import EmailSender
from api.shared.api.db import CategoryInDB, PlayerInDB
from api.shared.api.fftt_api import get_player_fftt
from api.shared.api.custom_decorators import (
    during_registration,
    after_registration_start,
)
import api.shared.api.api_errors as ae
from api.shared.models import Category, ContactInfo, Player, FfttPlayer, Entry
...
async def api_public_register_entries(
    ...
    ) > config["MAX_ENTRIES_PER_DAY"] + (player_in_db.gender == "F"):
        raise ae.InvalidDataError(
            origin=origin,
    ...
    try:
        ...
        EmailSender(
            sender_email=config["USKB_EMAIL"],
            password=config["USKB_EMAIL_PASSWORD"],
        ).send_email(
            ...
            f""": <a href="{config["TOURNAMENT_URL"]}/public/deja_inscrit/{licence_no}">cliquer ici</a>.<br><br>"""  # noqa: E501
